...
function add(date, duration, options) {
	...
	const _date = toDate(date, options?.in);
	const dateWithMonths = months$1 || years ? addMonths(_date, months$1 + years * 12) : _date;
	const dateWithDays = days$1 || weeks ? addDays(dateWithMonths, days$1 + weeks * 7) : dateWithMonths;
	const minutesToAdd = minutes + hours * 60;
	const msToAdd = (seconds + minutesToAdd * 60) * 1e3;
...
*/
function clamp(date, interval$1, options) {
	const [date_, start, end] = normalizeDates(options?.in, date, interval$1.start, interval$1.end);
...
function daysToWeeks(days$1) {
	const result = Math.trunc(days$1 / daysInWeek);
...
function normalizeInterval(context, interval$1) {
	const [start, end] = normalizeDates(context, interval$1.start, interval$1.end);
	return {
		start,
		end
	};
...
function eachDayOfInterval(interval$1, options) {
	const { start, end } = normalizeInterval(options?.in, interval$1);
...
* // ]
*/
function eachWeekendOfInterval(interval$1, options) {
	const { start, end } = normalizeInterval(options?.in, interval$1);
...
function formatDistance(laterDate, earlierDate, options) {
	...
	if (minutes < 2) if (options?.includeSeconds) if (seconds < 5) return locale.formatDistance("lessThanXSeconds", 5, localizeOptions);
		...
		const days$1 = Math.round(minutes / minutesInDay);
		return locale.formatDistance("xDays", days$1, localizeOptions);
	} else if (minutes < minutesInMonth * 2) {
		months$1 = Math.round(minutes / minutesInMonth);
		return locale.formatDistance("aboutXMonths", months$1, localizeOptions);
	}
	months$1 = differenceInMonths(earlierDate_, laterDate_);
	if (months$1 < 12) {
...
function formatDistanceStrict(laterDate, earlierDate, options) {
	...
	const milliseconds$1 = earlierDate_.getTime() - laterDate_.getTime();
	const minutes = milliseconds$1 / millisecondsInMinute;
	const timezoneOffset = getTimezoneOffsetInMilliseconds(earlierDate_) - getTimezoneOffsetInMilliseconds(laterDate_);
	const dstNormalizedMinutes = (milliseconds$1 - timezoneOffset) / millisecondsInMinute;
	...
	if (unit === "second") {
		...
		const days$1 = roundingMethod(dstNormalizedMinutes / minutesInDay);
		return locale.formatDistance("xDays", days$1, localizeOptions);
	} else if (unit === "month") {
		const months$1 = roundingMethod(dstNormalizedMinutes / minutesInMonth);
...
function formatRFC3339(date, options) {
	...
	if (fractionDigits > 0) {
		const milliseconds$1 = date_.getMilliseconds();
...
function getDaysInMonth(date, options) {
	...
	const lastDayOfMonth$1 = constructFrom(_date, 0);
...
*/
function intervalToDuration(interval$1, options) {
	const { start, end } = normalizeInterval(options?.in, interval$1);
	const duration = {};
	const years = differenceInYears(end, start);
	if (years) duration.years = years;
	const remainingMonths = add(start, { years: duration.years });
	const months$1 = differenceInMonths(end, remainingMonths);
	if (months$1) duration.months = months$1;
	const remainingDays = add(remainingMonths, { months: duration.months });
	const days$1 = differenceInDays(end, remainingDays);
...
function intlFormatDistance(laterDate, earlierDate, options) {
	...
	if (!options?.unit) {
		...
		if (Math.abs(diffInSeconds) < secondsInMinute) {
			...
			unit = "hour";
		} else if (Math.abs(diffInSeconds) < secondsInWeek && (value = differenceInCalendarDays(laterDate_, earlierDate_)) && Math.abs(value) < 7) unit = "day";
			...
			unit = "month";
		} else if (Math.abs(diffInSeconds) < secondsInYear) if (differenceInCalendarQuarters(laterDate_, earlierDate_) < 4) {
...
var DateParser = class extends Parser {
	...
	validate(date, value) {
		const year = date.getFullYear();
		const isLeapYear$1 = isLeapYearIndex$1(year);
		const month = date.getMonth();
...
* // => true
*/
function isWithinInterval(date, interval$1, options) {
	const time = +toDate(date, options?.in);
	const [startTime, endTime] = [+toDate(interval$1.start, options?.in), +toDate(interval$1.end, options?.in)].sort((a, b) => a - b);
...
function milliseconds({ years, months: months$1, weeks, days: days$1, hours, minutes, seconds }) {
	...
	if (months$1) totalDays += months$1 * (daysInYear / 12);
...
function millisecondsToHours(milliseconds$1) {
	const hours = milliseconds$1 / millisecondsInHour;
...
function millisecondsToSeconds(milliseconds$1) {
	const seconds = milliseconds$1 / millisecondsInSecond;
...
function monthsToQuarters(months$1) {
	const quarters = months$1 / monthsInQuarter;
...
function monthsToYears(months$1) {
	const years = months$1 / monthsInYear;
...
function roundToNearestHours(date, options) {
	...
	const roundedHours = getRoundingMethod(method)(hours / nearestTo) * nearestTo;
...
function roundToNearestMinutes(date, options) {
	...
	const method = options?.roundingMethod ?? "round";
	const roundedMinutes = getRoundingMethod(method)(minutes / nearestTo) * nearestTo;
...
*/
function setMilliseconds(date, milliseconds$1, options) {
	const _date = toDate(date, options?.in);
...
function sub(date, duration, options) {
	const { years = 0, months: months$1 = 0, weeks = 0, days: days$1 = 0, hours = 0, minutes = 0, seconds = 0 } = duration;
	const withoutMonths = subMonths(date, months$1 + years * 12, options);
	const withoutDays = subDays(withoutMonths, days$1 + weeks * 7, options);
	const minutesToSub = minutes + hours * 60;
	const msToSub = (seconds + minutesToSub * 60) * 1e3;
